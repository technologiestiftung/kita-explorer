var marker_kita_el = document.createElement('div');
    ...
    function updateKitas(){
      ...
      geojson.features.forEach(function(d,i){
        ...
        if(searchterm.length >= 3 && show){
          ...
          for(var key in fields){
            var obj = (typeof fields[key]);
            if(obj == 'string'){
              if((fields[key].toLowerCase().trim()).indexOf(searchterm) >= 0){
                tshow = true;
              }
            }
          }
      ...
      filterElements.age.bars.forEach(function(b,i){
        b.datum((i==0)?acceptance.under:acceptance.over).transition()
          .attr('x2', function(d,i){ return scales.age(d.count); });
        });
    ...
    function processFilter(){
      ...
      var openSelect = openListItem.append('select').on('change', function(d){
      ...
      var closeSelect = closeListItem.append('select').on('change', function(d){
